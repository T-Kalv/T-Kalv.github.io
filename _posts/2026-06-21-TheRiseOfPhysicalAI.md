...
After spending time working with multispectral computer vision payloads and embedded systems, it became apparent how different it is to process data on a screen versus the physical, real-world interaction. For the past decade, Artificial Intelligence has been strictly "disemmbodied", where it lives inside servers in Data Center, processing text and generating pixels on a screen. However, the next frontier of machine learning isn't just about thinking, it's about acting especially in the real, physical world.
...
We tend to assume that just because an Large Language Model (LLM) can write a complex Python script or pass a high level exam such as the bar, it possesses this so called "Artificial General Intelligence" (which is a hypothetical type of AI that can match or surpass all human capabilities on a variety of cognitive benchmarks).

## Core Ideas
### VLA (Vision-Language-Action) Models
![VLA Architecture](/images/VLA.png)

The current breakthrough in modern Physical AI comes from scaling up the Transformer architecture used by LLMs to an architecture known as VLAs (Vision-Language-Action) models in order to handle multi-modal inputs. LLMs would work by predicting the next word but VLAs models instead takes in a continuous stream of sensor data and outputs specific motor controls.
...
The Perception-Action loop is a fundamental concept in robotics where Physical AI operated in a continuous, real-time feedback loop where it perceives the environment and takes actions based on those perceptions. 

For example, a robot must constantly update its understanding of the world in real-time. If a a computer vision model running on a Humanoid robot detects an object held in its hands is slipping, the model must recalculate its trajectory in milliseconds. This requires running lots of neural networks on edge devices simultaneously. This means that 
